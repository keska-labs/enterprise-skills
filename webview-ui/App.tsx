...
export function App(): React.JSX.Element {
  ...
  const [mainTab, setMainTab] = useState<MainTab>("enabled");
  ...
  const enabledTabCategories = useMemo(() => {
    if (!state) {
      return [];
    }
  ...
  const filteredEnabledTabCategories = useMemo(() => {
    if (!searchQuery.trim()) {
      return enabledTabCategories;
    }
  ...
  }, [enabledTabCategories, searchQuery]);

  const hasEnabledTabSkills = enabledTabCategories.some((c) => c.skills.length > 0);
  ...
  const onConnect = () => vscode.postMessage({ type: "connectRepo" });
  const onSyncNow = () => {
    setIsSyncing(true);
    setSyncMessage(null);
  ...
  const onTabRowKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    if (e.key === "ArrowRight" && mainTab === "enabled") {
      e.preventDefault();
      setMainTab("browse");
      ...
      {!isConnected ? (
        ...
        <>
          <Header
            ...
            onChangeRepo={onConnect}
            onSyncNow={onSyncNow}
          />
          {syncMessage && syncFailed ? (
            <div className={`status-banner${syncFailed ? " error" : " success"}`} role="status">
              <span className={`status-dot${syncFailed ? " is-error" : " is-ok"}`} aria-hidden />
          ...
          {connectionHealth === "auth_required" && (
            <div className="status-banner error" role="alert">
              ...
              <button type="button" className="inline-action" onClick={() => vscode.postMessage({ type: "connectRepo" })}>
                Sign in
              </button>
            </div>
          )}
          ...
          >
            <button
              ...
              role="tab"
              id="tab-enabled"
              className={`tab-button${mainTab === "enabled" ? " active" : ""}`}
              aria-selected={mainTab === "enabled"}
              aria-controls="tab-panel-enabled"
              onClick={() => setMainTab("enabled")}
            >
              Enabled
            </button>
            <button
          ...
          {mainTab === "enabled" ? (
            <div id="tab-panel-enabled" role="tabpanel" aria-labelledby="tab-enabled">
              {hasEnabledTabSkills ? (
                <>
                  {filteredEnabledTabCategories.length === 0 ? (
                    <p className="no-results">Nothing matches “{searchQuery}”. Try a shorter phrase.</p>
                  ) : (
                    filteredEnabledTabCategories.map((category) => (
                      <CategoryGroup
                        key={category.name}
                        category={category}
            ...
            <div id="tab-panel-browse" role="tabpanel" aria-labelledby="tab-browse">
              <section className="callout-card callout-card--subtle">
                <div className="callout-body">
                  ...
                  <p className="callout-text">
                    Custom registries already load the full catalog on the <strong>Enabled</strong> tab. Switch back to filter and enable skills there.
                  </p>
                  <button type="button" className="button cta-button callout-cta" onClick={() => setMainTab("enabled")}>
                    Go to Enabled
                  </button>
                </div>
              </section>
            ...
            <div id="tab-panel-browse" role="tabpanel" aria-labelledby="tab-browse" className="browse-layout">
              <section className="surface-card tree-card" aria-label="Repository folders">
                <div className="surface-card-header">
                  ...
                  <p className="surface-card-sub">Folders load on demand from GitHub.</p>
                </div>
              ...
              <section className="surface-card search-card" aria-label="Full catalog search">
                <div className="surface-card-header">
                  <h2 className="surface-card-title">Search entire catalog</h2>
                  <p className="surface-card-sub">Indexes the repo once per search, then filters locally.</p>
                </div>
                <div className="surface-card-body">
                  <div className="catalog-search-field">
                    <label className="catalog-search-label" htmlFor="catalog-search-input">
                      Query
                    </label>
                    <div className="catalog-input-row">
                      <input
                        ...
                        type="search"
                        placeholder="e.g. security, deploy, docs…"
                        value={catalogQuery}
                    ...
                    <p id="catalog-search-hint" className="catalog-search-hint subtle">
                      Enter at least two characters. Large repositories may take a few seconds the first time.
                    </p>
                  </div>
                  {catalogSearchResults && catalogSearchResults.length > 0 ? (
                    <CategoryGroup
                      variant="results"
                      category={{ name: "Matches", skills: catalogSearchResults }}
                      optedInSkills={state?.optedInSkills ?? []}
                      onToggle={onToggle}
                    />
                  ) : catalogQuery.trim().length >= 2 && catalogSearchResults !== null && catalogSearchResults.length === 0 && !catalogSearching ? (
                    <p className="no-results no-results--left">No skills matched. Try another keyword.</p>
                  ) : null}
                </div>
              </section>
            </div>
