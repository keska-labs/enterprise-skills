...
- **Recommended tab:** **Refresh** and **Ask the Agent** stay in the toolbar above the loading state. Refresh cancels any in-flight ranking (via the recommendation cancellation token) and starts again; the AI-ranking badge appears once a result arrives. While an LLM ranks skills, output streams live into the webview with a collapsible transcript.
