...
- **Recommendations LLM chain**: Panel rankings use `vscode.lm` first (when an LM provider such as Copilot is available), then optional `@cursor/sdk` / OpenAI / Anthropic keys — see [`src/services/LlmSkillRecommender.ts`](./src/services/LlmSkillRecommender.ts). **Cursor IDE does not expose built-in models via `vscode.lm` today**, so many Cursor users see **heuristic** rankings unless they configure a key or use **Ask the Agent** with the Cursor plugin subagent.
- **Same repo, two products**: The VS Code extension ships alongside a **Cursor plugin** using [`.cursor-plugin/plugin.json`](./.cursor-plugin/plugin.json) (required by Cursor) and agent markdown under [`agents/`](./agents/) at the repo root—[official layout](https://cursor.com/docs/reference/plugins). Keep plugin agents separate from `src/` and `webview-ui/`.
