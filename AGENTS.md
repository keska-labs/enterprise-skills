...
- **Same repo, two products**: The VS Code extension ships alongside a **Cursor plugin** manifest at [`.cursor-plugin/plugin.json`](./.cursor-plugin/plugin.json) and assets under [`cursor-plugin/`](./cursor-plugin/). Keep plugin markdown separate from `src/` and `webview-ui/`.
